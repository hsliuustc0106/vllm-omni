...
class DiffusionModelRunner(OmniConnectorModelRunnerMixin):
    ...
    def __init__(
        ...
        self._weight_consumer_owner: WeightConsumerOwner | None = None
        self.prompt_embed_cache: Any | None = None
        self.input_batch: InputBatch | None = None
        self.model_memory_usage = 0
        runtime_mode = str(getattr(self.od_config, "host_weight_runtime_mode", "disabled"))
        initial_evidence = (
            not_requested_evidence()
            if runtime_mode == "disabled"
            else HostWeightResolutionEvidence(
                runtime_mode=runtime_mode,
                outcome="unavailable",
                events=("unavailable",),
                code="model_not_loaded",
                detail="model loading has not produced Host Weight Runtime evidence",
            )
        )
        self.host_weight_runtime_evidence = initial_evidence.to_dict()
    ...
    def get_host_weight_runtime_evidence(self) -> dict[str, object]:
        """Return this runner's immutable, JSON-safe resolution record."""

        evidence = dict(self.host_weight_runtime_evidence)
        owner = getattr(self, "_weight_consumer_owner", None)
        if owner is not None and owner.phase is WeightConsumerOwnerPhase.CONSUMER:
            consumer = owner.consumer
            diagnostics = getattr(consumer, "host_weight_diagnostics", None)
            if callable(diagnostics):
                evidence.update(diagnostics())
        evidence["ordinary_loader_fallback"] = self.host_weight_runtime_fell_back
        return evidence
    ...
    def load_model(
        ...
        if (
            getattr(self, "_weight_consumer_owner", None) is not None
            or getattr(
                self,
                "offload_backend",
                None,
            ...
            raise RuntimeError(
                "model runner still owns an offload lifecycle; call shutdown() successfully before loading again"
            )
        ...
        # Install the composition-owned HWR slot before constructing any
        # factory that can resolve, bind, or otherwise acquire weight state.
        load_config = LoadConfig()
        runtime_mode = str(getattr(self.od_config, "host_weight_runtime_mode", "disabled"))
        weight_owner: WeightConsumerOwner | None = None
        weight_consumer_factory: BuiltinOffloadWeightConsumerFactory | None = None
        if runtime_mode != "disabled":
            weight_owner = WeightConsumerOwner()
            self._weight_consumer_owner = weight_owner
            try:
                weight_consumer_factory = BuiltinOffloadWeightConsumerFactory(
                    self.od_config,
                    self.device,
                )
            except BaseException:
                weight_owner.close()
                if self._weight_consumer_owner is weight_owner:
                    self._weight_consumer_owner = None
                raise
        try:
            model_loader = DiffusersPipelineLoader(
                load_config,
                od_config=self.od_config,
                weight_consumer_owner=weight_owner,
                weight_consumer_factory=weight_consumer_factory,
            )
        except BaseException:
        ...
        def note_cleanup_failure(
            ...
            try:
                note = f"{action} also failed: {type(cleanup_exc).__name__}: {cleanup_exc}"
            except BaseException:
            ...
            try:
                ...
                # Cleanup reporting must never replace the load failure.
                pass

        def close_weight_owner(primary_exc: BaseException, action: str) -> bool:
            """Close the sole HWR authority without masking the primary error."""

            owner = self._weight_consumer_owner
            if owner is None:
                return True
            try:
                owner.close()
            except BaseException as cleanup_exc:
                note_cleanup_failure(primary_exc, action, cleanup_exc)
                return False
            if self._weight_consumer_owner is owner:
                self._weight_consumer_owner = None
            return True
        ...
        try:
            ...
            # HWR preparation publishes into the already installed owner.  The
            # loader and runner only borrow its result; there is no take-window
            # in which the prepared session lacks a cleanup authority.
            loader_host_weight_plan = model_loader.take_host_weight_plan()
            resolution_evidence = model_loader.take_host_weight_runtime_evidence()
            runtime_fell_back = bool(model_loader.host_weight_runtime_fell_back)
            if resolution_evidence is not None:
                self.host_weight_runtime_evidence = resolution_evidence
            self.host_weight_runtime_fell_back = runtime_fell_back
            any_offload = bool(
                self.od_config.enable_cpu_offload
                or self.od_config.enable_layerwise_offload
                or getattr(self.od_config, "enable_distributed_layerwise_offload", False)
            )
            if self.od_config.streaming_output and not getattr(
                self.od_config,
                "step_execution",
                False,
            ):
            ...
            if (
                ...
                and not self._supports_step_mode()
            ):
                raise ValueError(
            ...
            if runtime_mode != "disabled" and not any_offload:
                raise ValueError("Host Weight Runtime requires one CPU offload strategy")
            hwr_ready = False
            if weight_owner is not None and not runtime_fell_back:
                if weight_owner.phase is not WeightConsumerOwnerPhase.PREPARATION_RESULT:
                    raise RuntimeError("Host Weight Runtime preparation did not publish a closed owner result")
                hwr_ready = isinstance(
                    weight_owner.preparation_result,
                    PreparedSessionReady,
                )
                if not hwr_ready:
                    raise RuntimeError("Host Weight Runtime loader returned without a prepared session")
                if weight_owner.prepared_session.pipeline is not self.pipeline:
                    raise RuntimeError("Host Weight Runtime loader returned a different pipeline than the owner")
            elif weight_owner is not None:
                # The loader may close a resource-free UseLegacy result itself.
                # If it leaves the closed result for composition, drain it here
                # before constructing the independent legacy consumer.
                if weight_owner.phase is not WeightConsumerOwnerPhase.CLOSED:
                    weight_owner.close()
                if self._weight_consumer_owner is weight_owner:
                    self._weight_consumer_owner = None
                weight_owner = None
                weight_consumer_factory = None
            if hwr_ready and loader_host_weight_plan is not None:
                raise RuntimeError("the legacy host-weight plan and HWR session are mutually exclusive")
        except BaseException as exc:
            close_weight_owner(
                exc,
                "Closing the HWR owner after model-loader or runner preflight failure",
            )
            raise

        constructed_backend = None
        try:
            if hwr_ready:
                assert weight_owner is not None
                assert weight_consumer_factory is not None
                weight_consumer_factory.create_into(
                    owner=weight_owner,
                    pipeline=self.pipeline,
                )
                consumer = cast(BuiltinOffloadWeightConsumer, weight_owner.consumer)
                logger.info(
                    " Enabling Host Weight Runtime consumer: %s",
                    type(consumer).__name__,
                )
                consumer.enable_transactionally()
                constructed_backend = consumer.backend
                self.offload_backend = constructed_backend
            else:
                constructed_backend = get_offload_backend(
                    self.od_config,
                    device=self.device,
                    host_weight_plan=loader_host_weight_plan,
                )
                self.offload_backend = constructed_backend
                if constructed_backend is not None:
                    logger.info(
                        " Enabling offloader backend: %s",
                        constructed_backend.__class__.__name__,
                    )
                    constructed_backend.enable(self.pipeline)
        except BaseException as exc:
            if hwr_ready:
                close_weight_owner(
                    exc,
                    "Closing the HWR owner after consumer construction or enable failure",
                )
                if self._weight_consumer_owner is None:
                    self.offload_backend = None
            elif constructed_backend is not None:
                try:
                    constructed_backend.disable()
                except BaseException as cleanup_exc:
                    # Retain the backend as the only possible retry handle.
                    note_cleanup_failure(
                        exc,
                        "Disabling the partially initialized offload backend",
                        cleanup_exc,
                    )
                else:
                    self.offload_backend = None
        ...
        try:
            # Backend ownership remains transactional through every remaining
            # caller-side startup action.  Cache/prompt initialization and
            # logging may still raise after the HWR session was adopted.
            # Such a failure must disable the backend or retain it as the sole
            # retry handle before propagating the original error.
            ...
            if owner is not None:
                if close_weight_owner(
                    exc,
                    "Closing the HWR owner after runner startup failure",
                ):
                    self.offload_backend = None
            else:
                ...
                try:
                    ...
                    note_cleanup_failure(
                        exc,
                        "Disabling the offload backend after runner startup failure",
                        cleanup_exc,
                    )
                else:
                    if self.offload_backend is backend:
                        self.offload_backend = None
            raise

    def shutdown(self) -> None:
        """Drain offload work before releasing Host Weight Runtime mappings."""
        owner = getattr(self, "_weight_consumer_owner", None)
        if owner is not None:
            owner.close()
            if self._weight_consumer_owner is owner:
        ...
        if backend is None:
            return
        # Retain the only retry handle until terminal detach succeeds.  A
        # failed disable must never make an active session owner unreachable.
        backend.disable()
        if self.offload_backend is backend:
            self.offload_backend = None
